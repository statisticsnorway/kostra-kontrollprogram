...
final class ControlDubletter extends no.ssb.kostra.control.Control
{
  ...
  {
    String errorReport = "Kontroll 10, dubletter:" + lf + lf;
    errorReport += "\tAdvarsel: dubletter (lik kontoklasse * kapittel * sektor) " +
    "summeres sammen." + lf;
    ...
    int numOfRecords = errorMap.size();
      if (numOfRecords <= 10)
      {
        ...
        while (keyIterator.hasNext())
        {
          ...
          errorReport += lf + "\t- Kombinasjonen av kontoklasse, kapittel og sektor i record nr. " + firstOccurrence +
