...
final class ControlGyldigOrgNr extends no.ssb.kostra.control.Control {
  ...
  {
    String errorReport = lf + "Kontroll 22, gyldig organisasjonsnummer for særbedriftene:" + lf + lf;
    ...
    if (numOfRecords > 0)
    {
      errorReport += "\tFeil: Ugyldig organisasjonsnummer. " + lf + 
          ...
          "\tihht. Brønnøysundregistrets opplysninger pr. oktober " + Constants.kostraYear + ". " + 
          lf + "\tDersom foretaket ikke er registrert for regnskapsåret " + Constants.kostraYear +
          lf + "\tbes særskilt melding gitt til SSB";      
      if (numOfRecords <= 10)
      {
