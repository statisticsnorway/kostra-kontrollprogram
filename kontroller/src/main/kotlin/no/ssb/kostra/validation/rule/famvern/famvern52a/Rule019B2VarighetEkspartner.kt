...
class Rule019B2VarighetEkspartner : AbstractNoArgsRule<List<KostraRecord>>(
    ...
    override fun validate(context: List<KostraRecord>) = context.filter {
        ...
        fieldDefinitions.byColumnName(EKSPART_VARIGH_A_COL_NAME).codeExists(it[EKSPART_VARIGH_A_COL_NAME])
    }.map {
        val codeList =
            fieldDefinitions.byColumnName(EKSPART_VARIGH_A_COL_NAME).codeList
        createValidationReportEntry(
            messageText = "Det er oppgitt at primærklientens relasjon til viktigste deltager er ekspartner, men det " +
                    ...
                    "Fant '${it[EKSPART_VARIGH_A_COL_NAME]}', forventet én av: $codeList.",
