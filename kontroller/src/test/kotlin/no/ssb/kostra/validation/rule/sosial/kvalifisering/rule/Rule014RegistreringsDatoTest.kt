...
class Rule014RegistreringsDatoTest :
    BehaviorSpec({
        include(
            validationRuleNoContextTest(
                ...
                ForAllRowItem(
                    ...
                    kostraRecordInTest(regDatoString = "010120$twoDigitReportingYear"),
                ),
                ForAllRowItem(
                    "5 year diff between reportingYear and regDato",
                    kostraRecordInTest(regDatoString = "010120${twoDigitReportingYear - 5}"),
                    expectedErrorMessage =
                        "Feltet for 'Hvilken dato ble søknaden registrert ved NAV-kontoret?' " +
                            "med verdien (${"010120${twoDigitReportingYear - 5}"}) enten mangler utfylling, har ugyldig " +
                            "dato eller dato som er eldre enn 4 år fra rapporteringsåret (${argumentsInTest.aargang}). " +
                            "Feltet er obligatorisk å fylle ut.",
                ),
                ForAllRowItem(
                    "invalid regDato",
                    kostraRecordInTest(regDatoString = "a".repeat(8)),
                    expectedErrorMessage =
                        "Feltet for 'Hvilken dato ble søknaden registrert ved NAV-kontoret?' " +
                            "med verdien (${"a".repeat(8)}) enten mangler utfylling, har ugyldig dato eller dato " +
                ...
                ForAllRowItem(
                    ...
                    kostraRecordInTest(kommunenr = OSLO_MUNICIPALITY_ID, regDatoString = "010120${twoDigitReportingYear - 5}"),
                ),
                ForAllRowItem(
                    "For Oslo, invalid regDato",
                    kostraRecordInTest(kommunenr = OSLO_MUNICIPALITY_ID, regDatoString = "a".repeat(8)),
                ),
            ),
        )
    }) {
