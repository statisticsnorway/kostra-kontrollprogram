...
private int lines = 0;
  ...
  public String getErrorReport (int totalLineNumber)
  {
    ...
    if (Math.abs (sumForControl_8_a + sumForControl_8_b) >= MAX_DIFF)
    {
      ...
      errorReport += "\tKorreksjon: Rett opp slik at filen inneholder registrering av aktiva i balanse." + lf + lf;
    }

    if (sumForControl_8_b >= 0)    {
      errorReport += "\tFeil: Det er ikke registrert beløp i kapitler i passiva." + lf;
      errorReport += "\tKorreksjon: Rett opp slik at filen inneholder registrering av passiva i balanse." + lf + lf;
    }
